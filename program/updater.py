...
async def update_repo(_, message: Message):
    ...
    msg = await message.reply("🔄 `التحديث فالتقدم...`")
    ...
    if update_avail:
        ...
        return
    await msg.edit("bot is **up-to-date** with [main](https://github.com/islam-20555/islamtst/tree/main)", disable_web_page_preview=True)


@Client.on_message(command(["اعادة تشغيل", f"restart@{BOT_USERNAME}"]) & ~filters.edited)
@sudo_users_only
async def restart_bot(_, message: Message):
    msg = await message.reply("`جار اعادة التشغيل ...`")
